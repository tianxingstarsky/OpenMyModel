...
>
> OpenMyModel seamlessly tunnels your locally running llama.cpp models to your own cloud server through WebSocket, exposing them as industry-standard OpenAI-compatible endpoints. Whether you're a solo developer with spare GPU cycles, a hobbyist who loves self-hosting, or an operator building private inference nodes for a small team — OpenMyModel has everything you need. No public IP required, no complex ops: a single WebSocket tunnel turns your local model into a cloud API.
>
> #### Why Self-Host?
> Free online LLM platforms are everywhere, but nearly all serve aggressively quantized models — a "downgraded" version of intelligence. I've tested this firsthand: **Qwen 3.5 9B at INT8** running on a consumer GPU consistently outperforms the so-called "flagship" free-tier online services on logic and mathematical reasoning tasks. Free APIs compress quality for cost at scale — what you get is merely a shadow of the same model name. When you control precision and parameters yourself, every inference runs on real weights, and the difference exceeds expectations.
>
> #### Beyond Solo Use: Share & Monetize
> OpenMyModel was designed for more than personal use — it's built for compute sharing. Distribute API keys to teammates, friends, or community users, with per-key quota management and usage tracking. Idle GPUs are no longer sunk cost: start monetizing spare compute with a single `sk-` key.
...
│   │   ├── models/           # Data models
│   │   ├── pages/            # Pages (Home/Config/Chat/Cloud/Plugins)
│   │   ├── plugins/          # Web search plugin
...
│   ├── bridge_server.py      # WebSocket bridge + HTTP API
│   ├── server_manager.py     # llama-server process management
│   ├── config_manager.py     # Config profile management
│   ├── chat_handler.py       # Chat handling
│   └── requirements.txt
├── backend/                  # TypeScript cloud backend
│   ├── src/
│   │   ├── index.ts          # Express + WebSocket entry
│   │   ├── cli.ts            # Chinese CLI interaction
...
│   │       ├── websocket.ts  # WebSocket connection pool
│   │       └── auth.ts       # Authentication
│   ├── data/                 # Runtime data (gitignored)
│   ├── package.json
│   └── tsconfig.json
├── scripts/                  # Utility scripts
│   └── mock_node.js          # Mock node (for testing)
├── docs/                     # Documentation & screenshots
├── OpenMyModel.png           # README banner
├── logo.png                  # App icon
├── LICENSE
...
```bash
cd python
conda activate myenv              # or create a new env
...
### 4. CLI Management (Cloud Backend)
...
Wizard-driven setup for domain, password, and node status.
