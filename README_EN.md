...
- **Personal, provider and relay-only modes** — Provider mode adds email-code accounts, user-owned keys, Alipay top-ups and token billing. Relay-only mode routes each account only to its own nodes, with free access or time-based monthly subscriptions.
...
An administrator selects **Relay-only mode** in **System Settings**. The free plan requires an HTTPS public URL and SMTP. Monthly subscriptions also require Alipay RSA2 settings, a subscription price. Accounts remain unavailable until the selected plan's required settings are complete. Relay-only mode does not charge input or output tokens and has no separate cached-token price; users and administrators can still review request, token and access-frequency statistics.

Users register or sign in at `https://your-domain/console`, create a node in **Overview → My Nodes**, and copy its one-time node login token. Paste the token into the desktop app's **Cloud Connection** page, or put it in `NODE_TOKEN` for a headless Linux node. The local inference engine should still use its own node API key: the relay connector token authenticates the tunnel, while the engine API key protects the local inference service.

Users create public aliases in the same overview under **Model Mapping** and add routes to one or more of their own nodes. Upstream model names can differ between nodes: an alias such as `team-chat` can route to `Qwen3-8B` on one node and `my-local-qwen` on another. Gateway keys reveal only that account's available aliases, and requests can reach only that account's online, ready nodes. Other accounts' models, nodes and local engine keys are outside its routing scope. Admins can review accounts, node counts, orders and platform statistics, but cannot read or edit the local engine key stored on a user's node.
...
The default port is <code>3000</code>. You may set <code>ADMIN_PASSWORD</code> before startup instead. In **Cloud Connection**, enter the server address. Personal mode uses the administrator password; provider and relay-only modes use a one-time node login token from the web console. Public deployments should use HTTPS/WSS.
...
| Relay-only | Users register at <code>/console</code> and manage their own nodes, model aliases, routes and gateway keys. Requests go only to the user's own ready nodes. Token use is not billed; the optional monthly subscription grants time-based access. |
...
The first run downloads the model; startup time depends on model size and network speed. The vLLM API stays on the private Compose network and is not published on a host port. vLLM's API key protects only some endpoints; utilities such as <code>/tokenize</code> are unauthenticated, so do not add a port mapping. See [vLLM security guidance](https://docs.vllm.ai/en/latest/usage/security.html#api-key-authentication-limitations). The Connector waits for the model to become ready, then establishes its tunnel. In personal mode it uses the administrator credential. In provider mode it uses the user-issued node login token, and the admin registers that node's <code>NODE_API_KEY</code> and shared model routes; the user overview shows model billing revenue attributable to that node. In relay-only mode it also uses the user-issued token, while that user configures model mapping in <code>/console</code>. The admin does not receive the local <code>NODE_API_KEY</code> in relay-only mode.
