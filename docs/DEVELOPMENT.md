...
后端 `/` 返回自包含的深色状态页（无外链资源），`/status.json` 为机器可读数据，原接口清单 JSON 移至 `/api`。展示内容仅限公开聚合：在线节点数、并发容量/使用中与使用率、近期吞吐（完成请求输出流量的 EWMA，字节/秒）、累计请求与转发量、每个模型的节点数/并发/就绪状态。不包含节点 ID、名称、地址、密钥或对话内容。并发容量依赖桌面端上报 `slots`；进行中请求由后端中继计数实时统计。
