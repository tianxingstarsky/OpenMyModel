...
## 第八阶段：充值金额分精度校验（feat/llama-b10909-native-bridge，2026-09-25）
...
| --- | --- |
| 后端 TypeScript 构建 | 通过 |
| 后端测试 | 44 项通过；支付宝用例验证超过两位小数的充值输入被拒绝，不会静默舍入后创建不同金额的订单 |
| git diff check | 无空白错误 |

### 覆盖边界

- 测试使用签名密钥与 SQLite 临时夹具，没有连接真实支付宝或生产账户。

## 第七阶段：节点管理界面浏览器验收与支付宝交易幂等（分支 `feat/llama-b10909-native-bridge`，2026-09-25）

### 通过的检查

| 检查 | 结果 |
| --- | --- |
| 隔离本机预览 `/admin` 登录与节点管理页浏览器验收 | 页面正常加载；检查了节点 Key/统一调用者 Key 区分、节点状态概览、搜索与筛选区域 |
| `npm --prefix backend run build` | TypeScript 编译通过 |
| `npm --prefix backend test` | 44 项通过；支付宝用例验证 RSA2 身份与金额校验、重复通知幂等，以及同一支付宝交易号不能给第二笔内部订单入账 |
| `git diff --check` | 无空白错误 |

### 覆盖边界

- 浏览器验收使用独立临时数据库和本机预览凭据，没有连接真实节点、SMTP、支付宝或生产数据。
- 节点管理概念图是风格参考；上方浏览器验收检查的是实际页面，空节点状态与概念图中的演示数据布局不同。
